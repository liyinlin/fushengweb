...
(function (a) {
    a.fn.DB_tabMotionBanner = function (b) {
        ...
        return this.each(function () {
            var h = a(this);
            var k = h.find(".banner_imgSet");
            var r = h.find(".banner_imgSet li");
            var i = h.find(".banner_menuSet");
            var m = h.find(".banner_menuSet li");
            var e = h.find(".banner_bgSet li");
            ...
            function o() {
                for (var z = 0; z < r.length; z++) {
                    ...
                    if (p == z) {
                        A.show();
                        iCheck = z;
                        $(".hd").find("ul").find("li").each(function () { $(this).removeClass() });
                        $(".hd").find("ul").find("li").eq(z).addClass("on");
                        //$(".hd").find("ul").find("li").eq(z).trigger("click");
                        var x = A.find("img");
                        for (var w = 0; w < x.length; w++) {
                            var A = x.eq(w);
                            var v = c.motion[A.attr("class")];
            ...
            $(document).ready(function () {
                $(".banner_bgSet").find("li").each(function (i) {
                    if (i == 0) {
                        $(".hd").find("ul").append("<li class='on'>" + i + "</li>");
                    }
                    else {
                        $(".hd").find("ul").append("<li>" + i + "</li>");
                    }
                });
                $(".hd").find("ul").find("li").bind("click", function () {
